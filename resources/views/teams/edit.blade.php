<x-app-layout>
    ...
    <form method="POST" action="{{ route('management.teams.update', $team) }}" class="bg-white p-4 rounded shadow-sm">
        @csrf
        @method('PUT') {{-- Indica que é uma requisição de atualização --}}

        <div class="mb-4">
            <h2 class="h5 font-weight-bold">Informações da Equipe</h2>
            <p class="text-muted small">Atualize os dados da equipe abaixo.</p>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                <div class="form-floating">
                    {{-- Preenche com o valor atual da equipe --}}
                    <input type="text" class="form-control form-control-sm" id="name" name="name" value="{{ old('name', $team->name) }}" placeholder="Nome da Equipe" required maxlength="50">
            ...
            <div class="col-md-6 mb-3">
                <div class="form-floating">
                    {{-- Seleciona o status atual da equipe --}}
                    <select class="form-select form-select-sm" id="status" name="status" required>
                        <option value="" disabled>Selecione um status...</option>
                        <option value="active" @selected(old('status', $team->status) == 'active')>Ativa</option>
        ...
        <div class="row">
            <div class="col-md-12 mb-3">
                <div class="form-floating">
                    {{-- Preenche com a descrição atual --}}
                    <textarea class="form-control form-control-sm" id="description" name="description" placeholder="Descrição da Equipe" style="height: 100px" maxlength="500">{{ old('description', $team->description) }}</textarea>
        ...
        <div class="row">
            <div class="col-md-12 mb-3">
                <label for="select-users" class="form-label">Adicionar/Remover Usuários</label>
                <select id="select-users" name="users[]" multiple placeholder="Selecione os usuários...">
                    @php
                        // Cria um array com os IDs dos usuários já selecionados para facilitar a verificação
                        $selectedUserIds = old('users', $team->users->pluck('id')->toArray());
                    @endphp
                    @foreach ($users as $user)
                        {{-- O atributo 'selected' é adicionado se o ID do usuário estiver no array de selecionados --}}
                        <option value="{{ $user->id }}" @selected(in_array($user->id, $selectedUserIds))>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function setupCharCounter(inputId, counterId, maxLength) {
                ...
                if (input && counter) {
                    ...
                    // Adiciona o listener para atualizar enquanto digita
